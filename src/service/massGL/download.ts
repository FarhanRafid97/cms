...
import { BaseApiResponseMCRGL, FilterSearchParams, RequestPreBatch } from '@/types/globals';
import { toast } from 'sonner';

export const handleDownloadFileMassGL = async ({ fileName }: { fileName: string }) => {
  if (!fileName) {
    return;
  }
  try {
    const response = await axiosApiMCRGLCasa.get(`/download?filename=${fileName}`, {
      responseType: 'blob',
    });
...
export const handlerDownloadReportBatchMassGL = async ({
  searchParams,
  batchNumber,
}: {
  ...
  try {
    const response = await axiosApiMCRGLCasa.post('/batches/detail/download', requestBody, {
      responseType: 'blob',
    });
...
export const handlerDownloadReportPreBatchMassGL = async ({
  searchParams,
  batchNumber,
}: {
  searchParams: FilterSearchParams;
  ...
  try {
    const response = await axiosApiMCRGLCasa.post('preBatches/detail/download', requestBody, {
      responseType: 'blob',
    });
