...
export const handlerDownloadReportBatchMassDebetBPUM = async ({
  searchParams,
  batchNumber,
}: {
  ...
  try {
    const response = await axiosApiMassDebetBPUMV2.post('/batches/detail/download', requestBody, {
      responseType: 'blob',
    });
...
export const handlerDownloadReportPreBatchMassDebetBPUM = async ({
  searchParams,
  batchNumber,
}: {
  searchParams: FilterSearchParams;
  ...
  try {
    const response = await axiosApiMassDebetBPUMV2.post(
